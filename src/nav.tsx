...
//       const birthdaysToday = customers.filter(customer => format(new Date(customer.dob), 'MM-dd') === todayStr);
//       const upcoming = customers.filter(customer => {
//         const dobThisYear = new Date(today.getFullYear(), new Date(customer.dob).getMonth(), new Date(customer.dob).getDate());
//         return isAfter(dobThisYear, today) && isBefore(dobThisYear, endOfMonth);
//       });
//       const past = customers.filter(customer => {
//         const dobThisYear = new Date(today.getFullYear(), new Date(customer.dob).getMonth(), new Date(customer.dob).getDate());
//         return isBefore(dobThisYear, today) && isAfter(dobThisYear, startOfMonth);
//       });
...
//       ) : ( 
//         <p className='text-background'>No birthdays today</p>
//       )}
      
//       <h2>Upcoming Birthdays</h2>
//       {upcomingBirthdays.length > 0 ? (
//         upcomingBirthdays.map(customer => (
//           <p key={customer.id} className='text-background'>
...
//           <p key={customer.id} style={{ color: 'red' }} className='text-background'>
//             {customer.name} - {format(new Date(customer.dob), 'MMMM dd')}
//           </p>
//         ))
//       ) : (
